...
describe Solargraph::GemPins do
  it 'can merge YARD and RBS' do
    ...
    yard_pins = described_class.build_yard_pins([], gemspec)
    rbs_map = Solargraph::RbsMap.from_gemspec(gemspec, nil, nil)
    pins = described_class.combine yard_pins, rbs_map.pins
  ...
  it 'does not error out when handed incorrect gemspec' do
    ...
    expect { described_class.build_yard_pins([], gemspec) }.not_to raise_error
  end
end
