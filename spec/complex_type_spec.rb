...
describe 'YARD type specifier list parsing' do
  context 'with https://www.rubydoc.info/gems/yard/file/docs/Tags.md#type-list-conventions compliance' do
    ...
    # Regression test for https://github.com/castwide/solargraph/pull/1259#issuecomment-5192210594
    #
    # A Hash{} type parsed with a comma-separated (unparenthesized union)
    # key keeps key_types split across multiple ComplexType elements (see
    # the 'parses multiple key/value types' example above). If that type's
    # tag is later regenerated by code that no longer has hash_parameters?
    # set (e.g. a UniqueType rebuilt with parameters_type: :list, as can
    # happen when generics substitution recreates a Hash-named type from
    # a context type's key_types/subtypes), the substring generator must
    # not fall back to the 2-parameter Hash<K, V> notation, since that
    # can't represent more than one key or value type and produces a
    # string that fails to reparse.
    ...
    # A key_types/subtypes array can also have exactly one element that is
    # itself a multi-item union ComplexType (as generics substitution can
    # produce -- see UniqueType#resolve_generics, which replaces a single
    # generic<Key> slot with ComplexType.new(context_type.key_types)).
    # key_types.length == 1 alone isn't enough to guarantee the <K, V>
    # notation is safe to use.
  ...
  context 'when offering type queries orthogonal to YARD spec' do
    ...
    context 'when allowing users to define their own generic types' do
      ...
      it 'resolves generic parameters on a tuple using <()>' do
        ...
        expect(type.tag).to eq('Array<Array(String, Integer)>')
      end
