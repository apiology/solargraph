...
module Solargraph
  class PinCache
    ...
    def cached? gemspec
      rbs_version_cache_key = cache_key_for(gemspec)
    ...
    def deserialize_combined_pin_cache gemspec
      ...
      #
      # Deliberately not written to combined_pins_in_memory: that index is
      # process-wide, so a provisional set stored there would go on being
      # served after the build that supersedes it.
      rbs_map = RbsMap.from_gemspec(gemspec, rbs_collection_path, rbs_collection_config_path)
    ...
    def build_combine_and_cache gemspec,
      ...
      log_cache_info(gemspec, rbs_version_cache_key,
                     ...
                     build_combined: build_combined,
                     out: out)
      cache_yard_pins(gemspec) if build_yard
    ...
    def cache_yard_pins gemspec
      ...
      pins = Yardoc.build_pins(gem_yardoc_path, gemspec)
