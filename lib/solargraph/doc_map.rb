...
module Solargraph
  ...
  class DocMap
    ...
    def cache_doc_map_gems! out, rebuild: false
      ...
      stdlib_specs = pin_cache.possible_stdlibs.map { |stdlib| workspace.find_gem(stdlib, out: nil) }.compact
    ...
    def dependencies out: $stderr
      @dependencies ||=
        begin
          all_deps = gemspecs.flat_map { |spec| workspace.gemspecs.fetch_dependencies(spec, out: out) }
    ...
    def required_gems_map
      @required_gems_map ||= requires.to_h { |path| [path, workspace.gemspecs.resolve_require(path)] }
    end
